...
const PAGES = [
  ['portfolio', 'Портфель', '<rect x="2" y="2" width="5" height="5" rx="1"></rect><rect x="9" y="2" width="5" height="5" rx="1"></rect><rect x="2" y="9" width="5" height="5" rx="1"></rect><rect x="9" y="9" width="5" height="5" rx="1"></rect>'],
  ...
  ['data', 'Данные', '<ellipse cx="8" cy="4" rx="5" ry="2"></ellipse><path d="M3 4v8c0 1.1 2.2 2 5 2s5-.9 5-2V4M3 8c0 1.1 2.2 2 5 2s5-.9 5-2"></path>'],
];
...
const state = {
  data: null,
  api: true,
  page: location.hash.replace('#', '') || 'portfolio',
  scenario: 'BASE',
  selected: null,
...
const chips = (c, base = null, dim = false) => {
...
const okCount = (c, scenario) => checksOf(c, scenario).filter((r) => r.ok).length;
const stChip = (ok, label) => `<span class="st ${ok ? 'ok' : 'fail'}">${label}</span>`;

// ---------- шапка и навигация ----------
function renderChrome() {
  const d = state.data;
  $('#nav').innerHTML = PAGES.map(([k, t, ic]) => `<a data-page="${k}" class="${k === state.page ? 'on' : ''}"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">${ic}</svg><span>${t}</span></a>`).join('');
  const sc = d.meta.scenarios;
  $('#scenario').innerHTML = Object.keys(sc).map((s) => `<span data-s="${s}" class="${s === state.scenario ? 'on' : ''}">${s} · ${fmt(sc[s].c0_max, 0)}</span>`).join('');
...
function renderPortfolio() {
  ...
  const pill = failing.length
    ? `<div class="pill bad"><span class="dot">${ICON.x}</span>${ok} из ${checks.length} · нарушено: ${failing.map((r) => `${CHECK[r.id]} ${fmt(r.fact, DEC[r.id])} ${r.op === '<=' ? '>' : '<'} ${fmt(r.threshold, THR_DEC[r.id] ?? 0)}`).join(', ')}</div>`
    : `<div class="pill"><span class="dot">${ICON.check}</span>${ok} из ${checks.length} ограничений выполнены</div>`;

  const t = d.meta.totals, w = d.meta.weights;
  const helpSug = `Инструмент перебирает все ${fmt(t.combinations, 0)} комбинаций «4 лота × режимы» через канонический <b>case_core.py</b>, оставляет ${t.stress_feasible} допустимых в STRESS и ранжирует их моделью выбора команды: min–max нормализация по допустимым вариантам, веса — ценность ${w.vpub}, c0 ${w.c0}, cash / OPEX ${w.kcash}, готовность ${w.readiness}, устойчивость ${w.resilience}, тираж ${w.scale}, запас по STRESS ${w.stress_margin}. Портфель не собирается вручную по лотам: выбирается одна из предложенных комбинаций (клик по строке), отличия показаны относительно выбранной: жёлтый лот — замена, жёлтая буква — другой режим. Выбранная комбинация — ${s.rank}-я из ${t.stress_feasible} по баллу. Серая строка — максимум ценности среди проходящих BASE, но она не проходит STRESS и не предлагается.`;
  const rows = [...d.suggestions, ...d.rejected.filter((id) => !d.suggestions.includes(id))].map((id) => {
    const c = combo(id), isSel = id === state.selected, dim = !c.ok.STRESS;
    const why = isSel ? 'выбрана' : describeChange(s, c) + (dim ? ' · не проходит STRESS' : '');
    return `<tr class="pick${isSel ? ' hl' : ''}${dim ? ' dim' : ''}" data-id="${esc(id)}"><td><span class="radio${isSel ? ' on' : ''}"></span></td><td>${chips(c, s)}</td><td class="why">${esc(why)}</td><td class="num">${fmt(c.metrics.c0)}</td><td class="num">${fmt(c.metrics.vpub)}</td><td class="num">${fmt(c.metrics.kcash, 2)}</td><td>${stChip(c.ok.BASE, c.ok.BASE ? 'PASS' : 'FAIL')}</td><td>${stChip(c.ok.STRESS, c.ok.STRESS ? 'PASS' : 'FAIL · ' + (checksOf(c, 'STRESS').find((r) => !r.ok)?.id === 'c0_limit' ? 'c0' : 'см. стресс'))}</td><td class="num">${isSel ? '<b>' + c.score.toFixed(2) + '</b>' : c.score.toFixed(2)}</td></tr>`;
  }).join('');

  const m = s.metrics, cons = d.meta.constraints, c0max = d.meta.scenarios[sc].c0_max;
  const tile = (title, val, unit, reqW, lblA, lblB) => `<div class="tile"><div class="tl">${title}</div><div class="tv">${val}<small>${unit}</small></div><div class="seg"><i class="req" style="width:${(reqW * 100).toFixed(1)}%"></i><i class="sur" style="width:${((1 - reqW) * 100).toFixed(1)}%"></i></div><div class="seglbl"><span><i class="sw" style="background:#0b4a2e"></i>${lblA}</span><span><i class="sw" style="background:#1aa854"></i>${lblB}</span></div></div>`;
  ...
  const tiles = `<div class="tiles">
    ...
    <div class="tile"><div class="tl">Запас по c0</div><div class="two"><div><span>BASE</span><b class="${mB < 0 ? 'neg' : ''}">${fmt(mB)}</b></div><div><span>STRESS</span><b class="${mS < 0 ? 'neg' : ''}">${fmt(mS)}</b></div></div></div>
  </div>`;
  ...
  const group = (title, ids) => {
    const rs = checks.filter((r) => ids.includes(r.id));
...
${tiles}
<div class="card"><h2>Проверка ограничений<span class="u">сценарий ${sc}</span>${help(helpChecks)}</h2><div class="jobs2">${group('Состав портфеля', COMPOSITION)}${group('Финансовые и качественные пороги', THRESHOLDS)}</div></div>
...
function barChart(items) {
  ...
  g += `<line x1="${x0}" x2="${x1}" y1="${(base - floor * scaleY).toFixed(1)}" y2="${(base - floor * scaleY).toFixed(1)}" stroke="#c53a2f" stroke-dasharray="4 3" stroke-width="1.5"></line>`;
  items.forEach((c, i) => {
    const cx = x0 + step * (i + 0.5);
    const hv = c.metrics.vpub * scaleY, hc = c.metrics.cash * scaleY;
    g += `<rect x="${(cx - bw - 2).toFixed(1)}" y="${(base - hv).toFixed(1)}" width="${bw}" height="${hv.toFixed(1)}" rx="3" fill="#1e7c40"></rect><rect x="${(cx + 2).toFixed(1)}" y="${(base - hc).toFixed(1)}" width="${bw}" height="${hc.toFixed(1)}" rx="3" fill="#d9861a"></rect>`;
    g += `<text class="n" x="${(cx - bw / 2 - 2).toFixed(1)}" y="${(base - hv - 6).toFixed(1)}" text-anchor="middle" fill="#15211a">${fmt(c.metrics.vpub, 0)}</text>`;
...
<div class="legend"><span><i style="background:#1e7c40"></i>Общественная ценность</span><span><i style="background:#d9861a"></i>Поступления cash</span><span><i class="line"></i>порог ${fmt(floor, 0)}</span></div>`;
}
function c0Chart(items) {
  ...
  let g = `<rect x="${x0}" y="${top}" width="${(X(lo0) - x0).toFixed(1)}" height="${base - top}" fill="#e3f3e8"></rect><rect x="${X(lo0).toFixed(1)}" y="${top}" width="${(X(hi0) - X(lo0)).toFixed(1)}" height="${base - top}" fill="#fff2d8"></rect>`;
  if (X(hi0) < x1) g += `<rect x="${X(hi0).toFixed(1)}" y="${top}" width="${(x1 - X(hi0)).toFixed(1)}" height="${base - top}" fill="#fbe6e3"></rect>`;
  g += `<text class="xl" x="${((x0 + X(lo0)) / 2).toFixed(1)}" y="30" text-anchor="middle" fill="#0c5a2a">≤ ${fmt(lo0, 0)} · STRESS и BASE</text><text class="xl" x="${((X(lo0) + X(hi0)) / 2).toFixed(1)}" y="30" text-anchor="middle" fill="#6b4700">${fmt(lo0, 0)}–${fmt(hi0, 0)} · только BASE</text>`;
  g += `<line x1="${X(lo0).toFixed(1)}" x2="${X(lo0).toFixed(1)}" y1="${top - 4}" y2="${base + 4}" stroke="#c98500" stroke-width="2"></line><line x1="${X(hi0).toFixed(1)}" x2="${X(hi0).toFixed(1)}" y1="${top - 4}" y2="${base + 4}" stroke="#c53a2f" stroke-width="2"></line>`;
  items.forEach((c, i) => {
    const y = top + rowH * (i + 0.7), cx = X(c.metrics.c0), ok = c.ok.STRESS;
    g += `<text class="xl" x="200" y="${(y + 4).toFixed(1)}" text-anchor="end" fill="#15211a">${lotsOf(c)} · ${modesOf(c)}</text><line class="grid" x1="${x0}" x2="${x1}" y1="${y.toFixed(1)}" y2="${y.toFixed(1)}"></line><circle cx="${cx.toFixed(1)}" cy="${y.toFixed(1)}" r="6" fill="${ok ? '#0f8a3c' : c.ok.BASE ? '#c98500' : '#c53a2f'}" stroke="#fff" stroke-width="2"></circle>`;
    const right = cx + 70 <= x1; // подпись справа от точки, у правого края — слева
    g += right ? `<text class="n" x="${(cx + 11).toFixed(1)}" y="${(y + 4).toFixed(1)}" fill="#15211a">${fmt(c.metrics.c0)}</text>` : `<text class="n" x="${(cx - 11).toFixed(1)}" y="${(y + 4).toFixed(1)}" text-anchor="end" fill="#15211a">${fmt(c.metrics.c0)}</text>`;
  });
...
<div class="legend"><span><i style="background:#0f8a3c;border-radius:50%"></i>оба сценария</span><span><i style="background:#c98500;border-radius:50%"></i>только BASE</span></div>`;
}
function renderCompare() {
  ...
  const helpTable = `${items.length} комбинаций, посчитанных по одним правилам. Балл — взвешенная сумма нормированных (min–max по ${d.meta.totals.stress_feasible} допустимым в STRESS) критериев с весами: ценность ${w.vpub}, c0 ${w.c0}, cash / OPEX ${w.kcash}, готовность ${w.readiness}, устойчивость ${w.resilience}, тираж ${w.scale}, запас по STRESS ${w.stress_margin}. Подсвечена выбранная комбинация; строка с FAIL по STRESS — отвергнутая альтернатива с максимумом ценности.`;
  const rows = items.map((c) => `<tr class="${c.id === s.id ? 'hl' : ''}"><td>${chips(c)}</td><td class="num">${fmt(c.metrics.c0)}</td><td class="num">${fmt(c.metrics.opex)}</td><td class="num">${fmt(c.metrics.vpub)}</td><td class="num">${fmt(c.metrics.cash)}</td><td class="num">${fmt(c.metrics.kcash, 2)}</td><td class="num">${fmt(c.metrics.t_rep, 3)}</td><td>${stChip(c.ok.BASE, c.ok.BASE ? 'PASS' : 'FAIL')}</td><td>${stChip(c.ok.STRESS, c.ok.STRESS ? 'PASS' : 'FAIL · c0')}</td><td class="num">${c.id === s.id ? '<b>' + c.score.toFixed(2) + '</b>' : c.score.toFixed(2)}</td></tr>`).join('');
  return `<div class="top"><h1>Сравнение вариантов</h1><div class="pill${okS === items.length ? '' : ' warn'}"><span class="dot">${ICON.check}</span>${okS} из ${items.length} проходят STRESS</div></div>
<div class="row2">
<div class="card"><h2>Ценность и поступления<span class="u">в год</span>${help('Общественная ценность vpub — синтетическая шкала кейса в усл. млн руб./год, не деньги; с денежными поступлениями cash не складывается. Пунктир — минимум по кейсу. Cash = якорные × k_anchor + коммерческие × k_commercial за год. Под столбцами — состав и режимы в порядке лотов.')}</h2>${barChart(items)}</div>
<div class="card"><h2>Стартовые затраты и лимиты<span class="u">млн руб.</span>${help(`Положение c0 каждой комбинации относительно лимитов: ≤ ${fmt(d.meta.scenarios.STRESS.c0_max, 0)} проходит STRESS и BASE, ${fmt(d.meta.scenarios.STRESS.c0_max, 0)}–${fmt(d.meta.scenarios.BASE.c0_max, 0)} — только BASE, выше — не проходит ни один сценарий. Границы включительно.`)}</h2>${c0Chart(items)}</div>
</div>
<div class="card"><h2>Комбинации в сравнении${help(helpTable)}</h2>
<table><tr><th>Состав и режимы</th><th class="num">c0</th><th class="num">OPEX</th><th class="num">Ценность</th><th class="num">Cash</th><th class="num">Cash / OPEX</th><th class="num">t_rep</th><th>BASE</th><th>STRESS</th><th class="num">Балл</th></tr>${rows}</table></div>`;
}
...
function renderStress() {
  ...
  const cell = (r) => {
    ...
    const dec = r.id === 'kcash_floor' || r.id === 't_rep_floor' ? 2 : r.id === 'vpub_floor' ? 0 : 1;
  ...
  const rows = items.map((c) => {
    ...
    const cells = cols.map((k) => cell(rs[k]));
    const badCols = cols.filter((k, i) => cells[i].cls === 'bad');
    const weakest = badCols.length ? badCols.map((k) => short[k]).join(' · ') + ' · нарушение' : cols.filter((k, i) => cells[i].cls === 'thin').map((k) => short[k]).join(' · ') || short[cols[cells.map((x) => x.rel).indexOf(Math.min(...cells.map((x) => x.rel)))]];
  ...
  const helpMatrix = `Факт минус порог, рядом доля от порога. Зелёный — запас ≥ ${Math.round(thin * 100)} %, жёлтый — тонкий запас или граница, красный — нарушение. Состав (4 лота, архетипы, группы) выполнен у всех и в таблицу не вынесен. «Слабое место» — нарушенное или самое тонкое условие.`;
  let actions = '';
  if (fail) {
    ...
    actions = `<div class="row2 w">
<div class="card"><h2>Что можно сделать<span class="u">${lotsOf(fail)} · ${modesOf(fail)} · не проходит STRESS</span>${help('Правило кейса: лимит бюджета не меняет исходную стоимость лотов, c0 снижается только сменой режима или состава. Перевод лота из A в B даёт −5 % его c0 и −18 % ценности, но лот перестаёт быть public core. Каждая строка пересчитана через case_core; в столбце STRESS — запас до лимита.')}</h2>
<table><tr><th>Действие</th><th class="num">c0</th><th class="num">Ценность</th><th>STRESS</th></tr>${rowsA}</table></div>
<div class="card"><h2>Замена: ${esc(change)}<span class="u">что меняется</span>${help('Цена управленческого решения при сокращении бюджета: разница между отвергнутой комбинацией с максимальной ценностью и выбранной. Решение принимает межрегиональный заказчик; договоры по исключённому лоту не заключаются до второго этапа.')}</h2>
...
${rowD('Стартовые затраты c0', fail.metrics.c0, s.metrics.c0, 1)}${rowD('Общественная ценность', fail.metrics.vpub, s.metrics.vpub, 0)}${rowD('Cash / OPEX', fail.metrics.kcash, s.metrics.kcash, 2)}${rowD('Лотов с public core', fail.metrics.public_core, s.metrics.public_core, 0)}
  ...
  return `<div class="top"><h1>Стресс · c0 ≤ ${fmt(d.meta.scenarios.STRESS.c0_max, 0)}</h1>${bad ? `<div class="pill bad"><span class="dot">${ICON.x}</span>${bad} из ${items.length} не проходит</div>` : `<div class="pill"><span class="dot">${ICON.check}</span>все ${items.length} проходят</div>`}</div>
<div class="card"><h2>Запас по каждому ограничению<span class="u"><i class="sw" style="background:#e3f3e8"></i>≥ ${Math.round(thin * 100)} % <i class="sw" style="background:#fff2d8;margin-left:10px"></i>тонкий запас <i class="sw" style="background:#fbe6e3;margin-left:10px"></i>нарушение</span>${help(helpMatrix)}</h2>
<table class="matrix"><tr><th>Вариант</th>${cols.map((k) => `<th>${head[k]}</th>`).join('')}<th>Слабое место</th></tr>${rows}</table></div>${actions}`;
}

// ---------- сборка ----------
const ctx = { state, esc, fmt, help, icon: ICON, toast: (...a) => toast(...a), render: () => render(), reload: async () => { state.data = await loadDashboard(); state.selected = state.data.selected; localStorage.removeItem('kp.selected'); render(); } };
function render() {
  renderChrome();
...
document.addEventListener('click', async (e) => {
  ...
  if (nav) { state.page = nav.dataset.page; location.hash = state.page; render(); return; }
  const sc = e.target.closest('#scenario span');
  if (sc) { state.scenario = sc.dataset.s; render(); return; }
  ...
  if (!e.target.closest('.pop')) document.querySelectorAll('.help.open').forEach((o) => o.classList.remove('open'));
...
window.addEventListener('hashchange', () => { const p = location.hash.replace('#', ''); if (p && p !== state.page) { state.page = p; render(); } });
...
(async () => {
  ...
  if (!PAGES.some(([k]) => k === state.page)) state.page = 'portfolio';
  render();
})();
