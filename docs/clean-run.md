...
пустой папке из свежего клона и повторена на актуальной ветке (коммит
`8852d33`). Все команды ниже — ровно те, что выполнялись; результат записан
как есть. Шаги 1–4 и запуск сервера воспроизведены на `8852d33`; экранные
шаги 5–10 записаны по прогону Playwright на более раннем коммите — с тех пор
снимки экранов перерисовываются байт в байт теми же, то есть состав и цифры
интерфейса не менялись.

## Команды

```bash
git clone --branch claude/keen-einstein-csj511 https://github.com/pmixay/hackathon-1109 kosmo && cd kosmo
python -m venv .venv
...
|---|---|---|
| 1. clone | клон ветки в пустую папку | ok, `8852d33` |
